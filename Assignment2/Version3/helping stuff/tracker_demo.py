...
if __name__ == "__main__":
	...
	cap = cv.VideoCapture('../../data/obj01.mp4')
	...
	while(True):
		...
		if tracked_features.shape[0]>0:		# in case I have something within my array of features to trak
			...
			if(len(np.logical_not( fb_good.flatten()) == True) > 0):
			...
			for feature_idx in range(tracked_features.shape[0]):
				...
				cv.circle(mask, pos, 10, (255,255,255), thickness=-1)
		...
		if(corners.shape[0] > 0): #tracked_features = np.vstack((tracked_features, corners))
			# stack arrays in sequence vertically, for the coordinate of the cumulative features
